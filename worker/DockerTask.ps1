...
$ImageName = "example-voting-app-worker"

$FullyQualifiedImageName = "${RegistryUsername}${ImageName}"
...
function Clean () {
    ...
    $shellCommand = "docker-compose -f '$composeFilePath' -p $ProjectName rm -f"
    Write-Verbose "Executing: $shellCommand"
    Invoke-Expression "cmd /c $shellCommand `"2>&1`""
    if ($LastExitCode -ne 0) {
...
function Run () {
    ...
    $conflictingContainerIds = $(docker ps -a | select-string -pattern ":5000->" | foreach { Write-Output $_.Line.split()[0] })
    ...
    $shellCommand = "docker-compose -f '$composeFilePath' up -d"
    Write-Verbose "Executing: $shellCommand"
    Invoke-Expression "cmd /c $shellCommand `"2>&1`""
    Write-Verbose "Executed: $shellCommand"
    if ($LastExitCode -ne 0) {
        Write-Error "Failed to start the container(s)"
    }
    Write-Verbose "Before Open Site. OpenSite: ${OpenSite}"
    if ($OpenSite) {
        OpenSite
    }
}
...
function OpenSite () {
    ...
    if (-not $RemoteDebugging)
    {
        ...
        Write-Host "Waiting 10 seconds to get ready"
        Start-Sleep 10
    }
}
...
function GetUrl () {
  $hostName="http://docker:5000"
  Write-Verbose $hostName
  return $hostName
  #  $hostName="http://docker"
  #  if (-Not [System.String]:: IsNullOrWhiteSpace($Machine)) {
  #      $hostName="http://$(docker-machine ip $Machine)"
  #  }
...
function WaitForUrl ([string]$uri) {
    Write-Verbose "Opening site $uri " -NoNewline
    $status = 0
    $count = 0
...
function GetComposeFilePath([string]$folder) {
    ...
    if ($Environment -eq "Release") {
        ...
        $composeFileName = "docker-compose.yml.$Environment"
    }
